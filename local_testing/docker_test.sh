...
# Determine Docker TTY flags - only use -it when a TTY is available
if [ -t 1 ]; then
    DOCKER_TTY_FLAGS="-it"
else
    DOCKER_TTY_FLAGS=""
...
if [[ "$MODE" == "record_fixtures" ]]; then
    ...
    docker run --rm $DOCKER_TTY_FLAGS -v "$(pwd)":/home/runner/work/shell-tester/shell-tester shell-tester-ash make record_fixtures
    exit 0
fi
...
run_one() {
    ...
    docker run --rm $DOCKER_TTY_FLAGS -v "$(pwd)":/home/runner/work/shell-tester/shell-tester "$image" make "$make_target"
}
